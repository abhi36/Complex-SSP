<?php
    ...
    function setMedDetails($cellData, $index, $rowData, $pk){
        return $pk;
    }
    function hasEditOption($p){
        global $session, $rawTable, $formAction, $backEnd, $pk, $canEdit, $canDelete;
        ...
        $actionBtn = "";
        if($canEdit !== false){
            $actionBtn = '<a href="'.ADMIN_HOME.base64_encode($canEdit).'?edit='.$d.'&ref='.$session->getID().'" class="btn btn-primary">Redigera</a>';
        }
        return $actionBtn;
    }
    function hasDeleteOption($p){
        ...
        if($canDelete !== false){
            $actionBtn = '<form action="'.$formAction.'" class="form-horizontal pull-right" method="post" onsubmit="return confirm(\'Are you sure to delete?\')">
                ...
                <input type="hidden" name="rtakCsrf" value="'.$backEnd->getCSRFToken('request-model').'">
